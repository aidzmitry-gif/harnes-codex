...
def plan_actions(passport: object, parent_state: str) -> dict[str, Any]:
    ...
    for agent in agents:
        ...
        elif by_subgoal[subgoal_id]["status"] == "blocked":
            actions.append(_action("hold", subgoal_id, agent["id"], "subgoal_blocked"))
        else:
    ...
    for subgoal in subgoals:
        if subgoal["status"] == "blocked" and not any(agent["status"] == "blocked" for agent in agents_by_subgoal[subgoal["id"]]):
            actions.append(_action("hold", subgoal["id"], None, "subgoal_blocked"))

    if effective_parent_state == "running" and executable_chain and continuation_authorized and not any(action["action"] == "hold" for action in actions):
        available = cap - active
        ready = sorted(
            (item for item in subgoals if item["status"] == "ready" and not any(agent["status"] in ACTIVE_AGENT_STATUSES | {"blocked", "done"} for agent in agents_by_subgoal[item["id"]])),
            key=lambda item: (item["wave"], item["id"]),
        )
        for subgoal in ready[:available]:
            actions.append(
                _action(
                    "launch",
                    subgoal["id"],
